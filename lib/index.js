...
class PostCommandHook {
  ...
  async run(options = { skipErrors: false }) {
    ...
    for (let i = 0; i < this.plugins.length; i++) {
      ...
      if (runOnce) {
        if (storedPlugins.some((item) => item === pluginHash)) {
    ...
    if (runOnce) {
      this.conf.set("plugins", storedPlugins);
    }
    /* eslint-enable no-await-in-loop */

    if (skippedCount === 0) {
      if (runOnce) {
        if (this.plugins.length === alreadyExecutedCount) {
          consola.info(
            `There is no newly added plugins. A baseCommand and ${alreadyExecutedCount} plugins already have been executed.`
          );
          consola.info(
            "In order to run commands again you can deactivate runOnce setting by `postCommandHook.setRunOnce(false)`."
          );
        } else {
          consola.success(
            `Done! All newly registered plugins (${
              this.plugins.length - alreadyExecutedCount
            }) have been succeeded.`
          );
        }
      } else {
      ...
      consola.warn(
        `Some plugins have not been succeeded. Total: ${
          ...
          this.plugins.length - alreadyExecutedCount - skippedCount
        }, failed: ${skippedCount}.`
      );
    }
  }
